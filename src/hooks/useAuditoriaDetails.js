...
const useAuditoriaDetails = () => {
  ...
  const cadastrarAvaliacao = async (novaAvaliacao) => {
    try {
      // Verifica se o objeto tem cadavoperacionalId
      if (!novaAvaliacao || !novaAvaliacao.cadavoperacionalId) {
 ...
 // Dentro do seu useAuditoriaDetails.js (ou arquivo de hooks)
 const fetchUltimasAvaliacoes = async (auditoriaId, page = 1, limit = 10) => {
  try {
    ...
    if (
      response &&
      response.avaliacoes && // existe a chave 'avaliacoes'
      Array.isArray(response.avaliacoes) // e ela é um array
    ) {
      return {
    ...
    // Se for um objeto único (exemplo raro)
    ...
    // Se não for nem array de 'avaliacoes' nem objeto único
